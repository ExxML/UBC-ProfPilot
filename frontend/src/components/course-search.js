...
const CourseSearch = () => {
  ...
  const createTimeoutHandler = () => {
    return () => {
      setError(`Request timed out after 2 minutes. The API service likely ran out of memory while loading course professors. Try searching a course with fewer professors.`);
      setLoading(false);
