...
// Using a function to create the mapping on-demand to reduce initial memory footprint
let _departmentMappings = null;

const createDepartmentMappings = () => {
    if (_departmentMappings) return _departmentMappings;
    
    // Use Map for better memory efficiency and performance
    _departmentMappings = new Map([
        ["Accounting", 1],
        ["Administration", 248],
        ["Agriculture", 2],
        ["Anatomy", 678],
    ...
    return _departmentMappings;
};

// Export a proxy object that behaves like the original object but uses Map internally
export const DEPARTMENT_MAPPINGS = new Proxy({}, {
    get(target, prop) {
        const mappings = createDepartmentMappings();
        if (prop === Symbol.iterator || prop === 'keys' || prop === 'entries' || prop === 'values') {
            // Handle iteration methods
            if (prop === 'keys') return () => mappings.keys();
            if (prop === 'entries') return () => mappings.entries();
            if (prop === 'values') return () => mappings.values();
            if (prop === Symbol.iterator) return () => mappings.entries();
        }
        return mappings.get(prop);
    },
    has(target, prop) {
        return createDepartmentMappings().has(prop);
    },
    ownKeys(target) {
        return Array.from(createDepartmentMappings().keys());
    },
    getOwnPropertyDescriptor(target, prop) {
        if (createDepartmentMappings().has(prop)) {
            return {
                enumerable: true,
                configurable: true,
                value: createDepartmentMappings().get(prop)
            };
        }
    }
});