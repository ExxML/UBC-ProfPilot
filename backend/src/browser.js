...
class BrowserPool {
  ...
  async getBrowser() {
    ...
    if (this.persistentBrowser) {
      try {
        ...
        console.warn(
          "Persistent browser unhealthy, recreating:",
          error.message,
        );
    ...
    browser.on("disconnected", () => {
      console.log(
        "Browser disconnected, clearing persistent reference and context pool",
      );
      this.persistentBrowser = null;
  ...
  async createContext(browser) {
    ...
    try {
      ...
      console.warn(
        "Browser disconnected, cannot create context:",
        error.message,
      );
    ...
    await context.route("**/*", (route) => {
      ...
      try {
        ...
        if (
          /analytics|tracking|ads|facebook|google-analytics|doubleclick|googlesyndication|googletagmanager|hotjar|mixpanel|segment|amplitude|intercom|zendesk|twitter|linkedin|pinterest|instagram|tiktok|snapchat|reddit|youtube|vimeo/.test(
            urlLower,
          )
        ) {
          return route.abort("blockedbyclient");
    ...
    context.close = async function () {
      try {
        ...
        try {
          ...
          if (
            pool.contextPool.length < CONFIG.MAX_CONTEXT_POOL_SIZE &&
            !pool.isShuttingDown
          ) {
          ...
          console.warn(
            "Context unhealthy, not returning to pool:",
            healthError.message,
          );
        }
      } catch (error) {
    ...
    if (this.persistentBrowser) {
      try {
        ...
        await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Persistent browser close timeout")),
              30000,
            ),
          ),
        ]);
